...
class SVGAParser(private val context: Context) {
    ...
    fun parseFile(assetsName: String, callback: ParseCompletion) {
        try {
            val dir = File(Environment.getExternalStorageDirectory().absolutePath + "/QP/10001/gundonggangzhu.svga")
//                it stream = Files.newInputStream(Paths.get(assetsName)
            dir.inputStream().buffered(12 * 1024)?.let {
                parse(it, cacheKey("file:///assets/" + assetsName), callback, true)
            }
        } catch (e: Exception) {
