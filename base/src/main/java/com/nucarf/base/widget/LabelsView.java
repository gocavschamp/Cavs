...
public class LabelsView extends ViewGroup implements View.OnClickListener {
    ...
    private int label_normal_bg;//普通背景
    private int label_sel_bg;//选中背景
    private int label_radiu;//标签的圆角角度
    ...
    public LabelsView(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
    ...
    private void getAttrs(Context context, AttributeSet attrs) {
        if (attrs != null) {
            ...
            mTextSize = mTypedArray.getDimensionPixelOffset(R.styleable.labels_view_labelTextSize, 20);
            mTextPaddingLeft = mTypedArray.getDimensionPixelOffset(R.styleable.labels_view_labelTextPaddingLeft, 0);
            mTextPaddingTop = mTypedArray.getDimensionPixelOffset(R.styleable.labels_view_labelTextPaddingTop, 0);
            mTextPaddingRight = mTypedArray.getDimensionPixelOffset(R.styleable.labels_view_labelTextPaddingRight, 0);
            mTextPaddingBottom = mTypedArray.getDimensionPixelOffset(R.styleable.labels_view_labelTextPaddingBottom, 0);
            ...
            labelClickable = mTypedArray.getBoolean(R.styleable.labels_view_label_clickable, true);

            mTextSize = ScreenUtil.getRealWidth(context, mTextSize);
            mTextPaddingLeft = ScreenUtil.getRealWidth(context, mTextPaddingLeft);
            mTextPaddingTop = ScreenUtil.getRealWidth(context, mTextPaddingTop);
            mTextPaddingRight = ScreenUtil.getRealWidth(context, mTextPaddingRight);
            mTextPaddingBottom = ScreenUtil.getRealWidth(context, mTextPaddingBottom);
            mLineMargin = ScreenUtil.getRealWidth(context, mLineMargin);
            mWordMargin = ScreenUtil.getRealWidth(context, mWordMargin);

            int normalbgId = mTypedArray.getResourceId(R.styleable.labels_view_label_normal_bg, android.R.color.white);
            label_sel_bg = mTypedArray.getResourceId(R.styleable.labels_view_label_sel_bg, 0);
            label_sel_bg = mTypedArray.getResourceId(R.styleable.labels_view_label_sel_bg, 0);
            label_radiu = mTypedArray.getInt(R.styleable.labels_view_label_radiu, 0);

            label_normal_bg = getResources().getColor(normalbgId);
            mTypedArray.recycle();
        }
    }
...
//            }
     ...
     * @param labels
     */
    public void setLabels(List<String> labels, List<int[]> colors) {
        setLabels(labels, colors, new LabelTextProvider<String>() {
            @Override
     ...
     * @param labelsBgColor
     */
    public void setLabelsBgColor(int labelsBgColor) {
        this.label_normal_bg = labelsBgColor;
    }
     ...
     * @param <T>
     */
    public <T> void setLabels(List<T> labels, List<int[]> colors, LabelTextProvider<T> provider) {
        //清空原有的标签
        innerClearAllSelect();
        removeAllViews();
        mLabels.clear();
        ...
        if (labels != null) {
            ...
            for (int i = 0; i < size; i++) {
                if (null != colors) {
                    ...
                    addLabel(labels.get(i), i, color[0], color[1], provider);
                } else {
                    addLabel(labels.get(i), i, 0, 0, provider);
                }
            }
        }
    ...
    private <T> void addLabel(T data, int position, int norCol, int selCor, LabelTextProvider<T> provider) {
        ...
        label.setTextColor(norCol);
        //设置给label的背景(Drawable)是一个Drawable对象的拷贝，
        // 因为如果所有的标签都共用一个Drawable对象，会引起背景错乱。

        Drawable drawable = getStateListDrawable(norCol, selCor);
     ...
     * @return
     */
    private Drawable getStateListDrawable(int norCol, int selCol) {
        StateListDrawable stateListDrawable = new StateListDrawable();
        if (0 != norCol) {
            label_normal_bg = norCol;
        }
        if (0 != selCol) {
            label_sel_bg = selCol;
        }
        if (0 != label_sel_bg) {
            int[] stateHighlighted = new int[]{android.R.attr.state_pressed, android.R.attr.state_enabled};
            GradientDrawable drawable_press = new GradientDrawable();
            drawable_press.setCornerRadius(label_radiu);//圆角角度设置
            drawable_press.setColor(label_sel_bg);
            stateListDrawable.addState(stateHighlighted, drawable_press);
        }
...
//设置边框的厚度以及边框的颜色
        drawable_normal.setStroke(1,norCol);
        drawable_normal.setCornerRadius(label_radiu);//圆角角度设置
//        drawable_normal.setColor(label_normal_bg);
        stateListDrawable.addState(stateNormal, drawable_normal);
    ...
    public void onClick(View v) {
        if (v instanceof TextView) {
            ...
            if (mLabelClickListener != null) {
                ...
                Log.e("TAG", "position-label" + label + label.getTag(KEY_DATA) + "--position--" + (int) label.getTag(KEY_POSITION));
            }
        }
    }
    ...
    public void setLabelBackgroundDrawable(Drawable drawable) {
        ...
        for (int i = 0; i < count; i++) {
            TextView label = (TextView) getChildAt(i);
            label.setBackgroundDrawable(drawable.getConstantState().newDrawable());
        }
    }
